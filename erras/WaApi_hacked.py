...
class WaApiClient(object):
    ...
    def __init__(self, client_id, client_secret, log, debug=False):
    ...
    def authenticate_with_apikey(self, api_key, scope=None):
        ...
        self._token = self.parse_response(response)
    ...
    def log_raw_json(self, json_string):
        # to pretty-print the saved raw JSON, try:
        # python -m json.tool wild_apricot_response_YYYYMMDD-HHMMSS.json 
        filename = "wild_apricot_response_" + self.get_timestamp() + ".json"
        self.log.debug("Writing raw response JSON to filename %s" % filename)
    ...
    def parse_response(self, http_response):
        json_string = http_response.read().decode()
        if self.debug:
            self.log_raw_json(json_string)
        decoded = json.loads(json_string)
        # Spammy and slows things down, comment out when not debugging the json format
        # self.log.debug("raw json:")
        # self.log.debug("-------------------------------------------------------------------------------")
        # plog.debug(json_string)
        # self.log.debug("decoded json:")
        # self.log.debug("-------------------------------------------------------------------------------")
        # plog.debug(decoded)
        # self.log.debug("--------------------------------------------------------------------------------")
...
class ApiObject(object):
    ...
    def __init__(self, state, names=[]):
        self.__dict__ = state
#        Keep this commented out, it's fairly spammy and mainly for debugging.
#        self.log.debug("------------------------------")
#        self.log.debug("ApiObject.__init__(), state is:")
#        plog.debug(state)
#        self.log.debug("------------------------------")
        for key, value in vars(self).items():
            if isinstance(value, dict):
                names.append(key)
                self.__dict__[key] = ApiObject(value, names)
            elif isinstance(value, list):
                new_list = []
                for list_item in value:
                    if isinstance(list_item, dict):
                        new_list.append(ApiObject(list_item, names))
                    else:
