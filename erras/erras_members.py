...
class ErrasFiles(object):
    ...
    def get_contacts_headers(self):
        headers = ["Id", 
                   ...
                   "Status",
                   "Url"]
        for name in sorted(self.keypad_field_names + self.rfid_field_names):
            headers.append(name)
        return headers
    
    def strip(self, some_string):
        if some_string is not None:
            stripped_string = some_string.strip()  # Remove any leading/trailing whitespace
            stripped_string = stripped_string.lstrip("0") # Remove any leading zeroes to avoid python interpreting it as octal
            return stripped_string

    def contact_to_list(self, contact):
        field_values = dict()
        # combined list doesn't need to be sorted because it's going into a dict
        for name in self.keypad_field_names + self.rfid_field_names:   # TODO:  Should this be a set union?
        ...
        contact_list = [contact.Id,
                ...
                contact.Status,
                contact.Url]
#        for name in sorted(field_values):
#            print("For %s sorted key name is %s, value is %s" % (contact.DisplayName, name, field_values[name]))

            contact_list.append(field_values[name])
#        print("contact_to_list, returning: %s" % ", ".join(map(str, contact_list)))
        return contact_list
